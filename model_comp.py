...
def test_model(model, test_data_loader, filename):
    ...
    with torch.no_grad():
        for data_point in test_data_loader:
            ...
            if predicted != labels:
                plt.imshow(data_point[0][0].permute(1,2,0))
                plt.show()
                print(labels)
...
minitest_dataset = torchvision.datasets.ImageFolder(root=path_dir+'/minitest', transform=transform)
...
minitest_data_loader = torch.utils.data.DataLoader(minitest_dataset, batch_size=1, shuffle=False, num_workers=0)
...
with open(path_dir+'/epoch_losses/model_resnet_loss_2.json', "w") as outfile:
    outfile.write(json_object)

torch.save(model_resnet.state_dict(),path_dir+'/models/model_resnet_2.pt')

pred_list0, true_list0, conf_mat0 = test_model(model_resnet, test_data_loader, '/probs/problist_res__.csv' )
...
with open(path_dir+'/epoch_losses/model_eff_loss_2.json', "w") as outfile:
    outfile.write(json_object)
torch.save(model_efficient.state_dict(),path_dir+'/models/model_efficient_2.pt')

pred_list1, true_list1, conf_mat1 = test_model(model_efficient, test_data_loader, '/probs/problist_eff__.csv' )
...
with open(path_dir+'/epoch_losses/model_vit_loss_2.json', "w") as outfile:
    outfile.write(json_object)

torch.save(model_vit.state_dict(),path_dir+'/models/model_vit_2.pt')

pred_list2, true_list2, conf_mat2 = test_model(model_vit, test_data_loader, '/probs/problist_vit__.csv' )
